...
To get the table number with A that starts in Apendix section:
...
To get the table or figure number according to your interest:
- For table  `\renewcommand\thetable{A8}` will be the trick.
...
- For figure, `\renewcommand\thefigure{A9}` will be the trick.
...
\begin{figure}
 \renewcommand\thefigure{A8}
  \begin{center}
    \begin{tabular}{cc}
      \subcaptionbox{\label{Figure_4_a}Adaptation number}{\rotatebox{0}{\scalebox{.50}{\includegraphics{Figure_4_a}}}}
      &
      \subcaptionbox{\label{Figure_4_b}Adaptation indicator}{\rotatebox{0}{\scalebox{.50}{\includegraphics{Figure_4_b}}}}
    \end{tabular}
    \caption{\label{Figure:4}Boxplots for the number of adaptations (AdaptNum) and the adaptation indicator (AdaptInd)}
  \end{center}
\end{figure}
```
...
% Package
\usepackage{alphabeta,amsfonts,amsmath,amssymb,array,arydshln,booktabs,boxedminipage}
\usepackage{csquotes,enumerate,epsfig,epstopdf,gensymb,graphicx,indentfirst,latexsym}
\usepackage{multirow,newfloat,nomencl,oldgerm,rotating, setspace,siunitx,subcaption}
\usepackage{tabularx,textcomp,threeparttable,times,url,xcolor}
...
\DeclareSIUnit{\bdt}{\text{BDT}}
\DeclareSIUnit{\usd}{\text{USD}}
\DeclareSIUnit{\kg}{\text{kg}}
...
\author[2]{Y}
\author[3]{Z}
\affil[1]{\small A University}
\affil[2]{\small B University of Technology}
\affil[3]{\small C University}

\maketitle

% Abstract
\begin{abstract} Write abstract here
\end{abstract}

% Key words 
\textbf{Key Words}: ecological footprint; sustainable development
\end{titlepage}
...
% Additional information after references
\pagebreak

\subsection*{Author Contributions}
All authors contributed to the study conception and design. Material preparation, data collection and analysis were performed by X and Y. The first draft of the manuscript was written by X and Y. All authors commented on the manuscript and approved the final manuscript.
