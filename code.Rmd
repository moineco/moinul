...
If the above does not work, visit the following link and download **JabRef RPM**

<https://www.fosshub.com/JabRef.html>

Then by using the terminal, install the RMP file as:

For Ubuntu

```
sudo apt -y install [drag and drop the RPM file from the download folder]
```

For Fedora

```
sudo dnf -y install [drag and drop the RPM file from the download folder]
```
