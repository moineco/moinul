...
#### Manuscript by using quarto or rmardown
...
For writing an academic manuscript by using Latex, I prefer to use GNU Emacs environment. One of the main reason to use Emacs is the freedom to customize the codes. It also compatible with Linux (Ubuntu, Fedora), MacOS and Windows. So you will be able to create same environment of writing your manuscript across the board.

#### 3.1 For Ubuntu operating system

##### 3.1.1 Install texlive-full
...
We can install `texlive-full` using `apt` by running the following command:

```         
sudo apt -y install texlive-full
```

##### 3.1.2 Install Jabref
...
We can install `jabref` using `apt` by running the following command:

```         
sudo apt -y install jabref
```

3.1.3 Install Latexmk
...
We can install `latexmk` using `apt` by running the following command:

```         
sudo apt -y install latexmk
```

##### 3.1.4 Install Gnu Emacs
...
We can install the `emacs-gtk` package using `apt` by running the following command:

```
sudo apt -y install emacs-gtk
```

To operationalize the use of Emacs, we need to confirm that the following three files/folder (1) `.emacs`, (2) `.latexmkrc` and (3) `.emacs.d`  are existing in the `Home` directory of the Ubuntu operating system.

###### 3.1.4.1 Customize `.emacs`

By using your text editor in Ubuntu, create `.emacs` text file. Then you can write the following codes inside this text file:

```
```
Note: These codes are keep updating by the creator. Please contact directly to get the most recent version.

###### 3.1.4.2 Customize `.latexmkrc`

By using your text editor in Ubuntu, create `.latexmkrc` text file. Then you can write the following codes inside this text file:
...
###### 3.1.4.3 Load necessary `packages` listed on the `.emacs` file from ELPA/melpa/GNU or from other available repositories.

###### 3.1.4.4 Check your `.emacs.d` folder to confirm the existings packages are fully operational or not.

###### 3.1.4.5 Regulary update your emacs `packages`
